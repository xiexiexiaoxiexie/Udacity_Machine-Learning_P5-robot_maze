...
class Robot(object):
    ...
    def create_Qtable_line(self, state):
        ...
        for i in range(len(state)):
            #if self.Qtable.has_key(state[i]):
            if self.Qtable.has_key(state[i]):
                i=i
                #self.Qtable[state[i]]=state[i]
            else:
                self.Qtable[state[i]]={j: 0.0 for j in self.valid_actions}
                '''
    ...
    def choose_action(self):
        ...
        if self.learning:
            if is_random_exploration():
                ...
                self.action = max(self.Qtable[self.state],key=lambda x:self.Qtable[self.state][x])
                # TODO 7. Return action with highest q value
                #return None
        elif self.testing:
            #self.action = max(self.Qtable[self.state],key=self.Qtable[self.state].get)
            self.action = max(self.Qtable[self.state],key=lambda x:Qtable[self.state][x])
